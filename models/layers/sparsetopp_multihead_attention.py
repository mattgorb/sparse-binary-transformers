...
class SparseTopPMultiheadAttention(nn.MultiheadAttention):
    ...
    def __init__(self, embed_dim: int, num_heads: int,
        ...
        self.q_act_mask=torch.randperm(self.embed_dim*self.args.window_size)[:int((1-self.attention_prune_rate)*self.embed_dim*self.args.window_size)]
    ...
    def _forward_impl(self,
        ...
        '''if self.args.ablation:
            ...
            q.view(-1,q.size(0)*q.size(2) ).scatter_(-1, q_sort_ind[:,:prune_size ],torch.zeros_like(q.view(-1,q.size(0)*q.size(2) )) )



            k_sort_val, k_sort_ind = torch.sort(k.abs().view(-1,k.size(0)*k.size(2) ),dim=-1, descending=False)
            k.view(-1,k.size(0)*k.size(2) ).scatter_(-1, k_sort_ind[:,:prune_size ],torch.zeros_like(k.view(-1,k.size(0)*k.size(2) )) )


            v_sort_val, v_sort_ind = torch.sort(v.abs().view(-1,v.size(0)*v.size(2) ),dim=-1, descending=False)
            v.view(-1,v.size(0)*v.size(2) ).scatter_(-1, v_sort_ind[:,:prune_size ],torch.zeros_like(v.view(-1,v.size(0)*v.size(2) )) )
