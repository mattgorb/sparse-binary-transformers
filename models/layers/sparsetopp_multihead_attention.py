...
class SparseTopPMultiheadAttention(nn.MultiheadAttention):
    ...
    def _forward_impl(self,
        ...
        if self.args.ablation:
            ...
            print(q_sort_val[:,:prune_size ].int64().dtype)
            q.view(-1,q.size(0)*q.size(2) ).scatter(1, q_sort_val[:,:prune_size ].int64(),torch.zeros_like(q.view(-1,q.size(0)*q.size(2) )) )
            ...
            sys.exit()
            v_sort_val, v_sort_ind = torch.sort(v.abs().view(-1,v.size(0)*v.size(2)),dim=-1, descending=True)
            v.view(-1,v.size(0)*v.size(2) )[v_sort_ind[:,prune_size:]]=0
